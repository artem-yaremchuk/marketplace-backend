...
export const createAnimalSchema = Joi.object({
  animalName: Joi.string()
    ...
    .messages({
      ...
      "string.max":
        "Animal name must not be more than {#limit} characters long",
      "string.pattern.base":
        "Animal name can only contain letters, properly placed hyphens, apostrophes, and spaces",
  ...
  animalType: Joi.string()
    ...
    .messages({
      ...
      "string.max":
        "Animal type must not be more than {#limit} characters long",
      "string.pattern.base":
        "Animal type must contain only letters and properly placed spaces, without special characters or HTML tags",
  ...
  breed: Joi.string().trim().max(50).pattern(BREED_REGEX).required().messages({
    ...
    "string.pattern.base":
      "Breed name must consist only of letters, single spaces, properly placed hyphens, and apostrophes. Special characters and numbers are not allowed",
  ...
  animalLocation: Joi.string().trim().min(2).max(50).required().messages({
    ...
    "string.max":
      "Animal location must not be more than {#limit} characters long",
  ...
  adText: Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base":
        "Ad text can only contain letters, numbers, properly placed spaces, apostrophes, basic punctuation marks and emoji",
